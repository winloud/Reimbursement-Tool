...
export const getMaintenanceUpdateAction = ({
  portableInstall = false,
  busy = "",
  hasSelectedFile = false,
  hasPreview = false,
  previewCompatible = false,
  ...
  if (!portableInstall) {
    return { key: "unavailable", label: "更新不可用", interactive: false };
  }
  if (busy === "restart") {
    return { key: "restarting", label: "正在重启", interactive: false, busy: true };
  }
  if (restartRequired) {
    return { key: "restart", label: "重启程序", interactive: !busy };
  }
  if (busy === "update") {
    return { key: "installing", label: "正在安装", interactive: false, busy: true };
  }
  if (busy === "version-switch") {
    return { key: "switching", label: "正在切换", interactive: false, busy: true };
  }
  if (confirmation === "install") {
    return { key: "confirm-install", label: "确认安装", interactive: !busy, confirm: true };
  }
  if (confirmation === "switch") {
    return { key: "confirm-switch", label: "确认切换", interactive: !busy, confirm: true };
  }
  if (busy === "update-preview") {
    return { key: "previewing", label: "正在校验", interactive: false, busy: true };
  }
  if (!hasPreview) {
    return {
      key: hasSelectedFile ? "reselect" : "choose",
      label: hasSelectedFile ? "重新选择 ZIP" : "选择更新 ZIP",
      interactive: !busy,
    };
  }
  if (versionCurrent) {
    return { key: "current", label: "当前已安装", interactive: false };
  }
  if (!previewCompatible || (versionInstalled && !versionCompatible)) {
    return { key: "reselect", label: "重新选择 ZIP", interactive: !busy };
  }
  if (versionInstalled) {
    return { key: "switch", label: "切换到此版本", interactive: !busy };
  }
  return { key: "install", label: "安装更新", interactive: !busy };
};
