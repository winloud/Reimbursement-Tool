...
test("regular frontend keeps independent routes, flat navigation, and item-bound uploads", () => {
  ...
  assert.match(travelEditSource, /const nextReportDate = res\.data\?\.report_date \|\| ""/);
  assert.match(travelEditSource, /report_date: nextReportDate/);
  assert.match(editSource, /const nextReportDate = response\.data\?\.report_date \|\| ""/);
  assert.match(editSource, /report_date: nextReportDate/);
