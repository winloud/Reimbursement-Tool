...
- `wechat_qrcode/`：早期 OpenCV WeChatQRCode 模型文件。当前主包默认使用 `zxing-cpp`，这些模型不进入主 ZIP；仅在构建可选 `opencv-wechat-runtime-opencv-<opencv_package_version>-win_amd64.zip` 兼容运行时包时复用。
