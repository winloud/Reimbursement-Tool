...
- PDF 发票附件保留全部页；图片发票附件缩放居中放入 A5 横向页面
...
| Phase 4 | 待开始 | PDF 模板填充、中文大写金额、报销单预览图和合并 PDF 下载待开发 |
...
- [ ] `pdf_generator.py`：PDF 模板填充、多页逻辑、发票附件页合并
- [ ] `GET /api/reports/{id}/pdf/preview` 接口：生成仅报销单页预览图片，不改变状态
- [ ] `GET /api/reports/{id}/pdf` 接口：生成报销单页 + 全部发票附件页的合并 PDF，下载后标记为 `printed`
- [ ] 前端费用汇总卡片下新增「预览」「下载」按钮；未确认发票存在时按钮灰度显示，点击弹窗提示先确认发票
- [ ] 下载文件名按 `报销日期-出差事由-￥总金额.pdf` 生成
- [ ] 单元测试：中文大写转换全场景、Decimal 精度验证、PDF 分页、预览不改状态、下载改状态、未确认发票拦截
...
10. **PDF 输出限制**：存在未确认发票时不允许预览或下载 PDF；预览不改变状态，下载成功后标记为 `printed`
11. **文件删除联动**：删除发票或报销单时，软删除记录（设置 `deleted_at`），物理文件保留 30 天后清理
