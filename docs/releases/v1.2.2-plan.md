...
- [x] v1.2.2 GitHub Release workflow：run `28144114477` 成功，总耗时 `237s`；URL https://github.com/winloud/Reimbursement-Tool/actions/runs/28144114477。
- [x] v1.2.2 GitHub Release 资产校验：主包 `reimbursement-tool-v1.2.2-20260625.zip`（约 45.5 MB），OpenCV runtime `opencv-wechat-runtime-opencv-4.10.0.84-win_amd64.zip`（约 60.75 MB）；Release URL https://github.com/winloud/Reimbursement-Tool/releases/tag/v1.2.2。
- [x] v1.2.2 正式 ZIP 内容校验：发布后本地执行 metadata-only 校验，未下载远端主 ZIP；ZIP 内容结构由 GitHub Actions 的 `Validate local release ZIP` 步骤在上传前对本地构建产物完成校验。
- [x] v1.2.2 发布耗时采集：`scripts\collect_release_metrics.ps1 -RunId 28144114477` 已生成 JSON/Markdown 摘要。
