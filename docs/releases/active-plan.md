...
## 目标
- [x] 收集下一轮需求并确认版本范围。
- [x] 拉开发票上传入口与已上传发票列表的视觉层级。

## 范围
本次做：
- 将上传入口改为浅蓝操作带，保留拖拽、选中和 Ctrl+V 反馈；选中与拖入使用一致的上浮、阴影和扫光动效。
- 为已上传发票增加分区标题、数量提示和确认状态侧边色。
...
- [x] 发票上传入口使用浅蓝虚线操作带，选中与拖入状态共享上浮、阴影和扫光反馈；已上传发票使用独立标题、浅灰实体行和确认状态侧边色，避免两个区域视觉混淆。
...
- [x] 发票区域视觉分层验证：前端 `52 passed`，`npm run build` 通过；桌面双栏下上传操作带与发票分区层级清晰，390px 窄屏下页面无横向溢出，上传区尺寸约 `309×62px`。
- [x] 上传区选中动效统一验证：选中状态与拖入状态共享上浮、阴影和扫光反馈；前端 `52 passed`，`npm run build` 通过，仅保留现有大 chunk 警告。

### 已同步到 CHANGELOG
- [x] 发布治理简化、安全续跑和不可变 tag 规则已写入 `Unreleased`。
- [x] 发票上传入口与已上传发票视觉分层已写入 `Unreleased`。
