...
- 普通多文件上传逐个处理，重复项失败不影响其余文件继续上传；全部重复、部分重复和普通上传失败统一先展示“发票上传结果”弹窗，按已上传、重复、失败分类汇总。部分成功时由用户查看结果后选择继续确认已上传发票或稍后确认，不再直接跳过重复明细进入确认窗口。重复提示最多列出 3 个来源报销单的编号、事由、日期、人员和状态，超出时显示剩余数量。
...
- 普通批量上传中重复项不会留下数据库记录或临时附件；全部重复和部分重复使用同一种结果弹窗，部分成功时只有点击“继续确认”才进入确认队列，也可关闭弹窗稍后确认。长重复提示在桌面和 390px 宽度下完整换行且不造成页面横向溢出。
...
### 重要改动
- fix(ui): 普通发票批量上传的全部重复、部分重复和普通失败统一使用“发票上传结果”弹窗；重复项与真实失败分色展示，部分成功由用户查看汇总后选择继续确认或稍后处理，避免提示层级和出现顺序不一致。
...
### 验证记录
- 重复发票提示统一回归：`python -m pytest -q`，359 passed、2 skipped（7 个既有弃用警告）；`frontend` 下执行 `node --test src/**/*.test.js`，89 passed；`npm run build` 成功（Vite 6.4.2，1712 modules，仅有既有大 chunk 提示）。
- 重复发票提示统一浏览器验收：使用隔离临时数据库和测试 PDF，先上传一张发票后再次上传同一文件，全部重复显示“已上传 0 / 重复 1”和“知道了”；同时上传重复文件与新文件时显示“已上传 1 / 重复 1”，确认窗口未提前打开，点击“继续确认 1 张”后才进入发票信息确认。390×844 下长来源信息完整换行，结果弹窗和页面均无横向溢出，浏览器控制台无错误；未读取或修改用户报销数据。
- 全库重复发票本地预览包：`scripts/build_release.ps1 -PreviewBuild -Version 1.3.0 -PreviewSerial 001 -ReleaseDate 20260727 -SkipDependencyInstall -ReuseReleaseVenv` 成功，生成 `release/报销管理-v1.3.0-preview-20260727-001.zip`（45.23 MB）。
- preview-20260727-001 内容校验：264 个 ZIP 条目，启动器、版本目录、前端资源和清单均存在；应用版本为 `1.3.0-preview-20260727-001`，数据结构为 v4、支持 v1-v4；未包含 data、uploads、logs、browser-profile、vendor、window-state.json、测试目录或既有 release 产物；SHA-256 为 `532863A4865A852B35D3C9EBF7D5C71D40F12458509CC826FC0A2D8908D07866`。
...
### 已同步到 CHANGELOG
- 已在 `Unreleased / Fixed` 中记录普通发票全部重复、部分重复和普通失败统一使用上传结果弹窗，并允许部分成功时先查看汇总再继续确认。
