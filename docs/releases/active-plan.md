...
- [x] 数据迁移：实际便携目录副本迁移后核对库行数、附件哈希、备份和设置一致；失败可重试，旧目录不变。
- [x] 下载：PDF、批量 ZIP、数据导出、备份、诊断包覆盖保存成功、取消、重名、不可写路径及非法后端路径。
- [x] 安装矩阵：Windows 10/11 x64；在线包在已有 WebView2 或联网 bootstrap 环境可安装启动；断网且缺失 WebView2 时提示明确；卸载重装不删 AppLocalData 数据。
- [x] 更新矩阵：测试签名和测试 feed 完成预览版间升级、安装后重启、升级前备份、签名篡改拒绝、不兼容数据结构拒绝。
- [x] 业务烟测：报销单 CRUD、发票/附件上传与预览、PDF 生成与保存、数据导入导出、备份恢复、OpenCV 兼容模式。
