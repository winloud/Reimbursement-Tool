...
开发时重启前后端，统一双击或运行仓库根目录脚本：

```powershell
.\restart-dev.cmd
```

该脚本会在同一个控制台窗口中重启后端 `127.0.0.1:8000` 和前端 `127.0.0.1:5174`。输入 `q`、`quit`、`exit` 或 `stop` 后回车，或直接关闭窗口，都会关闭两个服务。

详细说明见：`docs/dev-server.md`。
