...
报销管理 V1.0 是一个本地单机运行的出差旅费报销管理工具，用于录入出差信息、管理发票、生成公司模板报销 PDF，并查看历史报销统计。
...
- 个性化设置：默认部门、出差人、途中补贴标准、PDF 填充字体
...
`_internal` 目录包含程序运行所需的前端文件、Python 运行库、PDF 模板、二维码识别模型和其他依赖。只复制 EXE 会导致程序无法启动。
