...
function Update-Readme {
    ...
    $text = Replace-Required -Text $text -Pattern "报销管理_\d+\.\d+\.\d+_x64-setup\.exe" -Replacement "报销管理_${Version}_x64-setup.exe" -Description "README Tauri example"
...
function Invoke-Preflight {
    ...
    & (Join-Path $PSScriptRoot "prepare_zip_release.ps1") @params
    if (-not $?) {
...
function Get-LatestReleaseRun {
    ...
    $json = & gh run list --workflow "Publish ZIP Release" --limit 50 --json databaseId,displayTitle,headBranch,headSha,event,status,conclusion,url,createdAt
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to list GitHub Actions runs."
    }
    $parsed = $json | ConvertFrom-Json
    $runs = if ($parsed -is [array]) { $parsed } else { @($parsed) }
    $exactRuns = @($runs | Where-Object { $_.displayTitle -eq "Publish ZIP Release $TagName" })
...
function Watch-ReleaseRun {
    ...
    Write-Host "==> Watch Publish ZIP Release run $($Run.databaseId)"
    & gh run watch $Run.databaseId --exit-status
    if ($LASTEXITCODE -ne 0) {
        throw "Publish ZIP Release workflow failed for run $($Run.databaseId). The release commit and immutable tag were preserved. Commit and push any workflow-only fix to $ReleaseBranch, then rerun this command. Actions: $($Run.url)"
    }
}
...
function Invoke-ReleaseAssetValidation {
    ...
    try {
        & (Join-Path $PSScriptRoot "validate_zip_release.ps1") @validationParams | Out-Null
        if (-not $?) {
...
function Invoke-ReleaseWorkflow {
    ...
    if ($TagWasJustPushed) {
        # ZIP 发布工作流仅手动触发，避免同一 tag 同时启动 Tauri 与 ZIP 发布任务。
        $run = $null
    }
    ...
    elseif ($run) {
        Write-Host "Previous Publish ZIP Release run $($run.databaseId) failed. Dispatching the current $ReleaseBranch workflow against immutable tag $TagName."
        $run = $null
    }

    $dispatchStartedAt = [DateTime]::UtcNow.AddSeconds(-10)
    Write-Host "==> Dispatch Publish ZIP Release for immutable tag $TagName"
    & gh workflow run "Publish ZIP Release" --ref $ReleaseBranch -f "tag=$TagName"
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to dispatch Publish ZIP Release for $TagName. The tag was not moved or deleted."
    }
    $run = Wait-NewReleaseRun -ReleaseCommit $ReleaseCommit -NotBeforeUtc $dispatchStartedAt -Attempts 60
    if (-not $run) {
        throw "Timed out waiting for the dispatched Publish ZIP Release workflow for $TagName."
    }
    Watch-ReleaseRun -Run $run
    return Get-LatestReleaseRun -ReleaseCommit $ReleaseCommit
}
