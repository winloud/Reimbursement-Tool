...
function Test-ReleasePrepared {
    ...
    if (-not (Test-FileContains -RelativePath "README.md" -Pattern "reimbursement-tool-v$escapedVersion-x64-setup\.exe")) {
        return $false
    }
...
function Update-Readme {
    ...
    if ($text -match "(?m)^# 报销管理 V\d+\.\d+\.\d+ 发布说明\s*$") {
        ...
        $text = Replace-Required -Text $text -Pattern "报销管理_(?:X\.Y\.Z|\d+\.\d+\.\d+)_x64-setup\.exe" -Replacement "reimbursement-tool-v${Version}-x64-setup.exe" -Description "README installer example"
        ...
        $nl = Get-NewLine -Text $text
        if ($text -notmatch "(?m)^## v\d+\.\d+\.\d+ 发布信息\s*$") {
            $text = Replace-Required -Text $text -Pattern "报销管理-v\d+\.\d+\.\d+-\d{8}\.zip" -Replacement "报销管理-v$Version-$ReleaseDate.zip" -Description "README ZIP example"
            $text = Replace-Required -Text $text -Pattern "报销管理_(?:X\.Y\.Z|\d+\.\d+\.\d+)_x64-setup\.exe" -Replacement "reimbursement-tool-v${Version}-x64-setup.exe" -Description "README installer example"
        }
        $releaseInfo = "# 报销管理${nl}${nl}## $TagName 发布信息${nl}${nl}发布日期：$ReleaseDateDisplay${nl}${nl}便携 ZIP：``报销管理-v$Version-$ReleaseDate.zip``${nl}Tauri 在线安装包：``reimbursement-tool-v${Version}-x64-setup.exe``"
